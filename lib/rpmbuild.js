...
function performBuild(ctx, cb){
    ...
    util.rpmbuild(ctx.specFileName, ctx.rpmRootDir, ctx, function(err, rpms){
        ctx.rpms = rpms;
        cb(err, ctx);
    });
}
                ...
                fs.stat(file.path, function(err, stats){
                    if (stats.isDirectory()){
                        ...
                        fsx.copy(file.path, relPath, function(err){
                            if(item.chmod){
                                fsx.chmod(file.path, item.chmod, function(err){ cb(err, file); });
                            }
                            else {
                                cb(err, file);
                            }
